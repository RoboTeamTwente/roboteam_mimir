...
RobotMesh::RobotMesh(const RobotSettings& settings) {
    ...
    for (int i = 0; i < angleSteps; ++i) {
        mesh.emplace_back(radius*sin(angle),radius*cos(angle),halfHeight);
        mesh.emplace_back(radius*sin(angle),radius*cos(angle),-halfHeight);
        angle+=angleStep;
    }
    //also append the front side still
    mesh.emplace_back(radius*sin(endAngleRad),radius*cos(endAngleRad),halfHeight);
    mesh.emplace_back(radius*sin(endAngleRad),radius*cos(endAngleRad),-halfHeight);


}

const std::vector<btVector3>& RobotMesh::hull() const {
    return mesh;
}
